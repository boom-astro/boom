...
const ZTF_TEST_PIPELINE_LSST_PRV_CANDIDATES: &str = "[{\"LSST.prv_candidates.0\": {\"$exists\": true}, \"$match\": {\"candidate.drb\": {\"$gt\": 0.5}, \"candidate.ndethist\": {\"$gt\": 1.0}, \"candidate.magpsf\": {\"$lte\": 18.5}}}, {\"$project\": {\"objectId\": 1, \"annotations.mag_now\": {\"$round\": [\"$candidate.magpsf\", 2]}}}]";
