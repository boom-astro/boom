...
// build the filter for the cone search, which is the filter + 
...
#[post("/query")]
async fn query(client: web::Data<Client>, body: web::Json<QueryBody>) -> HttpResponse {
    // body is a json object with:
    // - the query type (find, cone_search, sample, info)
    // - the query parameters that contains:
    //   - the filter
    //   - the projection
    //   - the catalog (collection)
    // then we have an optional kwargs object that can contains:
    // - the limit
    // - the skip
    // - the sort
    // - the max_time_ms
