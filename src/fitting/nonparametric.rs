...
pub fn fit_nonparametric(bands: &HashMap<String, BandData>) -> Vec<NonparametricBandResult> {
    ...
    for (band_name, band_data) in bands {
            ...
            for &nugget in &nugget_candidates {
                if let Some(gp_fit) = fit_egobox_gp(&times_arr, &mags_arr, theta_init, nugget) {
                    if let Ok(pred_at_obs_arr) = gp_fit.predict(&times_orig_2d) {
                        let pred_at_obs = pred_at_obs_arr.view();
                        let mut chi2 = 0.0;
                        ...
                        if let Ok(pred_grid_arr) = gp_fit.predict(&times_pred_2d) {
                            let pred_grid = pred_grid_arr.view();
                            let pred_grid_min =
                                pred_grid.iter().cloned().fold(f64::INFINITY, f64::min);
                            let obs_min = band_data
                                .values
                                .iter()
                                .cloned()
        ...
        if let Some(gp_fit) = best_gp {
            ...
            let pred = pred_arr.view().to_vec();

            // Estimate GP uncertainty from residuals
...
fn fit_egobox_gp(
    ...
    .theta_init(Array1::from_vec(vec![theta_init]))
...
fn compute_predictive_features(
    ...
    let y = gp.predict(&xq).unwrap().view().to_vec();
