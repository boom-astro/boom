use crate::{
    alert::{
        base::{get_schema_and_startidx, AlertError, AlertWorker, AlertWorkerError},
        decam, lsst,
    },
    conf,
    utils::{
...
pub const ZTF_LSST_XMATCH_RADIUS: f64 = (2.0_f64 / 3600.0_f64).to_radians(); // 2 arcseconds in radians
pub const ZTF_DECAM_XMATCH_RADIUS: f64 = (2.0_f64 / 3600.0_f64).to_radians(); // 2 arcseconds in radians
...
impl ZtfAlertWorker {
    ...
    async fn get_decam_matches(&self, ra: f64, dec: f64) -> Result<Vec<String>, AlertError> {
        let decam_matches = {
            let result = self
                ...
                .find_one(doc! {
                    "coordinates.radec_geojson": {
                        "$nearSphere": [ra - 180.0, dec],
                        "$maxDistance": ZTF_DECAM_XMATCH_RADIUS,
                    },
                })
                .projection(doc! {
                    "_id": 1
    ...
    async fn get_survey_matches(&self, ra: f64, dec: f64) -> Result<Document, AlertError> {
        ...
        let decam_matches = self.get_decam_matches(ra, dec).await?;
