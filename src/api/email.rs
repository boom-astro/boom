...
/// Shared shell for the "here is your one-time code" emails.
///
/// Both the sign-up activation code and the social sign-in email-verification
/// code use the identical branded layout; only the copy, the code label, the
/// call to action and the closing note differ. Keeping one template means the
/// two can't drift apart visually.
fn branded_code_email(
    intro: &str,
    code_label: &str,
    code: &str,
    cta_block: &str,
            ...
            <table width="100%" cellpadding="0" cellspacing="0" border="0">
              <tr><td align="center">
                <div style="display:inline-block;background:#f0f6ff;border:2px solid #4f8ef7;
                  ...
                  <p style="margin:0 0 4px;font-size:16px;font-weight:700;letter-spacing:2px;color:#4f8ef7;text-transform:uppercase;padding-bottom:10px;">{code_label}</p>
...
impl EmailService {
    ...
    /// Confirm an email address a user typed in while signing in with Google,
    /// GitHub, or ORCID.
    ///
    /// Needed when the provider gave us no email we can trust — ORCID users
    /// usually keep theirs private. The code proves the person controls the
    /// address before we attach it to an account.
    pub fn send_identity_verification_email(
        &self,
        to_email: &str,
        provider_name: &str,
        code: &str,
        ...
        let cta_block = if let Some(url) = webapp_url {
            ...
            format!(
                r#"<p style="margin:16px 0 8px;font-size:14px;color:#334155;">
                  Confirm via <code style="font-family:monospace;">curl</code>:
                </p>
                <pre style="background:#0f172a;color:#e2e8f0;padding:18px 20px;
                            border-radius:8px;font-size:13px;line-height:1.6;
                            overflow-x:auto;white-space:pre-wrap;word-break:break-all;margin:0 0 8px;">curl -X POST https://{domain}/babamul/oauth/verify \
     -H 'Content-Type: application/json' \
     -d '{{"ticket":"{ticket}","code":"{code}"}}'</pre>"#,
                domain = domain,
                ticket = ticket,
                code = code,
            )
        ...
        let email_body = branded_code_email(
            &format!(
                "You signed in with {}, which didn't share an email address with us. \
                 Use the code below to confirm this address so we can finish setting up \
                 your Babamul account.",
                provider_name
            ),
            "Confirmation Code",
            code,
        ...
        let email = Message::builder()
            ...
            .to(to_email
                .parse()
                .map_err(|e| format!("Invalid to address: {}", e))?)
            .subject("Confirm Your Email for Babamul")
            .header(ContentType::TEXT_HTML)
            .body(email_body)
            .map_err(|e| format!("Failed to build email: {}", e))?;
