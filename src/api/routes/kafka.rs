...
use crate::api::kafka::delete_acls_for_babamul_user;
...
/// Delete Kafka ACLs for a given Babamul user
#[utoipa::path(
    delete,
    path = "/kafka/acls/babamul/{user_email}",
    responses(
        (status = 200, description = "Kafka ACLs deleted successfully"),
    ),
    tags=["Kafka"]
)]
#[delete("/kafka/acls/babamul/{user_email}")]
pub async fn delete_babamul_kafka_acls(
    ...
    match delete_acls_for_babamul_user(&user_email) {
        Ok(_) => response::ok(
