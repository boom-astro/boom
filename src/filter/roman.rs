...
use crate::enrichment::{create_roman_alert_pipeline, fetch_alerts, RomanAlertForEnrichment};
use crate::filter::{
...
/// Builds Roman Alert packets from the provided filter results.
#[instrument(skip_all, err)]
pub async fn build_roman_alerts(
    ...
    for alert in alerts {
        ...
        // `band` is nullable in the RAPID schema; a point without one cannot be
        // placed on a lightcurve, so it is left out of the packet.
        let mut photometry = Vec::new();
        for (points, origin) in [
            (alert.prv_candidates.iter(), Origin::Alert),
            (alert.fp_hists.iter(), Origin::ForcedPhot),
        ] {
            for doc in points {
                let Some(band) = doc.band.as_ref() else {
                    continue;
                };
                photometry.push(Photometry {
                    ...
                    band: format!("roman{}", band),
                    origin: origin.clone(),
                    programid: 1, // RAPID publishes a single stream
                    survey: Survey::Roman,
                    ra: doc.ra,
                    dec: doc.dec,
                });
        ...
        let alert = Alert {
            ...
            survey: Survey::Roman,
            // Roman objects are cross-matched to ZTF/LSST in the aux `aliases`,
            // but the matched lightcurves are not bundled into the outgoing
            // packet yet (cf. LSST, which bundles ZTF).
...
/// Builds a MongoDB aggregation pipeline for Roman filter execution.
///
/// Augments the user filter pipeline with Roman aux lookups (prv_candidates,
/// fp_hists, cross_matches, aliases) based on which fields the filter
/// references.
pub async fn build_roman_filter_pipeline(
...
impl FilterWorker for RomanFilterWorker {
    #[instrument(err)]
    async fn new(
        config_path: &str,
        filter_ids: Option<Vec<String>>,
        ...
        let alert_cutout_storage = config.build_cutout_storage(&Survey::Roman).await?;

        let input_queue = "ROMAN_alerts_filter_queue".to_string();
        let output_topic = "ROMAN_alerts_results".to_string();

        let watchlist_projections = watchlist_projections(&config, &Survey::Roman);
    ...
    async fn refresh_filters(&mut self) -> Result<(), FilterWorkerError> {
        info!("refreshing ROMAN filters from database");
        ...
        .await?;
        info!(
            "refreshed ROMAN filters from database; now tracking {} filters",
            self.filters.len()
        );
        Ok(())
    }
    ...
    async fn process_alerts(&mut self, alerts: &[String]) -> Result<Vec<Alert>, FilterWorkerError> {
        ...
        // RAPID publishes a single stream (no programids), so the queue holds
        // bare candids.
