...
/// A stored WISE lightcurve point (prv_candidate or forced-phot), as fetched
/// from the aux collection to build the outgoing alert packet.
#[derive(Debug, Clone, serde::Deserialize, serde::Serialize)]
pub struct WisePhotometry {
    pub jd: f64,
    pub magpsf: f32,
    pub sigmapsf: f32,
    pub band: Band,
...
impl WisePhotometry {
    /// PSF magnitude -> flux (nJy) so the packet is consistent with the other
    /// surveys. WISE reports Vega-based mags, so this is the AB-zeropoint flux
    /// implied by the reported magnitude, not a recalibration.
...
/// WISE alert as fetched from the DB to build the outgoing alert packet.
#[derive(Debug, Clone, serde::Deserialize, serde::Serialize)]
pub struct WiseAlertForFilter {
    #[serde(rename = "_id")]
    pub candid: i64,
    ...
    #[serde(default)]
    pub prv_candidates: Vec<WisePhotometry>,
    #[serde(default)]
    pub fp_hists: Vec<WisePhotometry>,
}

fn create_wise_filter_alert_pipeline() -> Vec<Document> {
    vec![
        doc! {
            "$match": {
                "_id": {"$in": []}
...
pub async fn build_wise_alerts(
    ...
    let alerts: Vec<WiseAlertForFilter> = fetch_alerts(&candids, &alert_pipeline, alert_collection)
        .await
...
/// Builds a MongoDB aggregation pipeline for WISE filter execution.
pub async fn build_wise_filter_pipeline(
    ...
    if use_prv_candidates_index.is_some() {
        aux_add_fields.insert(
            ...
            fetch_timeseries_op("aux.prv_candidates", "candidate.jd", 5000, None),
        );
    }
    if use_fp_hists_index.is_some() {
        aux_add_fields.insert(
            "fp_hists".to_string(),
            fetch_timeseries_op("aux.fp_hists", "candidate.jd", 5000, None),
        );
    }
...
impl FilterWorker for WiseFilterWorker {
    #[instrument(err)]
    async fn new(
        config_path: &str,
        filter_ids: Option<Vec<String>>,
        ...
        let alert_cutout_storage = config.build_cutout_storage(&Survey::Wise).await?;

        let input_queue = "WISE_alerts_filter_queue".to_string();
        let output_topic = "WISE_alerts_results".to_string();

        let watchlist_projections = watchlist_projections(&config, &Survey::Wise);
        ...
        Ok(WiseFilterWorker {
            alert_pipeline: create_wise_filter_alert_pipeline(),
            alert_collection,
    ...
    async fn refresh_filters(&mut self) -> Result<(), FilterWorkerError> {
        info!("refreshing WISE filters from database");
        ...
        .await?;
        info!(
            "refreshed WISE filters from database; now tracking {} filters",
            self.filters.len()
        );
        Ok(())
    }
