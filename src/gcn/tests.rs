...
mod watchlist_validation_tests {
    ...
    #[test]
    fn test_watchlist_public_from_healpix_returns_none() {
        // WatchlistPublic::from_event should return None for HEALPix events
        use crate::api::routes::events::EventPublic;

        let event = GcnEvent {
            id: "hp-event".to_string(),
            source: GcnSource::Lvk,
            event_type: GcnEventType::GravitationalWave,
            trigger_time: 2460000.0,
        ...
        // EventPublic handles both, but geometry_type should be "healpix"
        let public = EventPublic::from_event(&event);
        assert_eq!(public.geometry_type, "healpix");
        assert!(public.ra.is_none());
        assert!(public.dec.is_none());
        assert!(public.error_radius.is_none());
    }
}
...
mod config_tests {
    use crate::conf::{
        GcnBackfillConfig, GcnCacheConfig, GcnConfig, GcnConsumerConfig, GcnExpirationConfig,
        WatchlistConfig,
    };
    ...
    fn test_gcn_config_defaults() {
        ...
        assert_eq!(config.expiration.default_days, 14);
        assert_eq!(config.backfill.lookback_days, 7);
        assert_eq!(config.backfill.lookahead_days, 7);
    ...
    fn test_gcn_config_full_deserialization() {
        ...
        assert!(config.enabled);
        assert_eq!(config.expiration.default_days, 21);
        assert_eq!(config.expiration.by_source.get(&GcnSource::Lvk), Some(&30));
        assert_eq!(config.expiration.by_source.get(&GcnSource::Swift), Some(&7));
        assert_eq!(config.backfill.lookback_days, 14);
        assert_eq!(config.backfill.lookahead_days, 3);
    ...
    fn test_consumer_config_defaults() {
        let config = GcnConsumerConfig::default();
    ...
    #[test]
    fn test_consumer_config_deserialization() {
        let json = r#"{
            "client_id": "my-client",
            "client_secret": "my-secret",
            "topics": ["gcn.classic.text.FERMI_GBM_FIN_POS", "gcn.notices.swift.bat.guano"]
        }"#;
        let config: GcnConsumerConfig = serde_json::from_str(json).unwrap();
    ...
    #[test]
    fn test_expiration_config_defaults() {
        let config = GcnExpirationConfig::default();
        assert_eq!(config.default_days, 14);
        assert!(config.by_source.is_empty());
    }

    #[test]
    fn test_expiration_config_with_all_sources() {
        let json = r#"{
            "default_days": 7,
            "by_source": {
                "lvk": 30,
        ...
        let config: GcnExpirationConfig = serde_json::from_str(json).unwrap();
        assert_eq!(config.default_days, 7);
        assert_eq!(config.by_source.len(), 7);
        assert_eq!(config.by_source.get(&GcnSource::Lvk), Some(&30));
        assert_eq!(config.by_source.get(&GcnSource::Custom), Some(&365));
        assert_eq!(config.by_source.get(&GcnSource::IceCube), Some(&10));
    }

    #[test]
    fn test_backfill_config_defaults() {
        let config = GcnBackfillConfig::default();
        assert_eq!(config.lookback_days, 7);
        assert_eq!(config.lookahead_days, 7);
    }

    #[test]
    fn test_backfill_config_deserialization() {
        let json = r#"{"lookback_days": 30, "lookahead_days": 14}"#;
        let config: GcnBackfillConfig = serde_json::from_str(json).unwrap();
    ...
    fn test_cache_config_defaults() {
        let config = GcnCacheConfig::default();
        assert_eq!(config.refresh_interval_secs, 60);
    }

    #[test]
    fn test_cache_config_deserialization() {
        let json = r#"{"refresh_interval_secs": 300}"#;
        let config: GcnCacheConfig = serde_json::from_str(json).unwrap();
    ...
    #[test]
    fn test_gcn_source_as_hashmap_key_roundtrip() {
        let mut by_source: HashMap<GcnSource, u32> = HashMap::new();
        by_source.insert(GcnSource::Lvk, 30);
        by_source.insert(GcnSource::Swift, 7);
        by_source.insert(GcnSource::EinsteinProbe, 21);

        let json = serde_json::to_string(&by_source).unwrap();
        let deserialized: HashMap<GcnSource, u32> = serde_json::from_str(&json).unwrap();

        assert_eq!(deserialized.len(), 3);
        assert_eq!(deserialized.get(&GcnSource::Lvk), Some(&30));
        assert_eq!(deserialized.get(&GcnSource::Swift), Some(&7));
        assert_eq!(deserialized.get(&GcnSource::EinsteinProbe), Some(&21));
    }

    #[test]
    fn test_gcn_source_hashmap_no_duplicates() {
        let mut map: HashMap<GcnSource, u32> = HashMap::new();
        map.insert(GcnSource::Lvk, 10);
        map.insert(GcnSource::Lvk, 20); // overwrite
        assert_eq!(map.len(), 1);
        assert_eq!(map.get(&GcnSource::Lvk), Some(&20));
    }
}
